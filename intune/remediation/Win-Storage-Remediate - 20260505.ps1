...
.\Win-Storage-Remediate-v3.ps1 -DryRun
...
# Toggle these to enable/disable individual cleanup activities between Regions 2A and 7B.
# $true = activity will be performed, $false = activity will not be performed
# Defaults are normally set to $true to preserve existing behaviour, unless explicitely enumerated
...
$Pref_ForceDISMWhenPendingOverride = $true
    ...
    try {
        ...
        if ($script:VerboseBuffer -and $script:VerboseBuffer.Count -gt 0) {
            foreach ($vb in $script:VerboseBuffer) {
                ...
                if (Test-Path -Path $tempLogFile) { Add-Content -Path $tempLogFile -Value $vbEntry -ErrorAction SilentlyContinue }
                else { Add-Content -Path $logFile -Value $vbEntry -ErrorAction SilentlyContinue }
            }
        ...
        if (Test-Path -Path $tempLogFile) {
            ...
            try { $null = Split-Path -Path $logFile -Parent; if ($null -and -not (Test-Path -Path (Split-Path -Path $logFile -Parent))) { New-Item -Path (Split-Path -Path $logFile -Parent) -ItemType Directory -Force | Out-Null } } catch {}
...
if (-not $Pref_RemoveOldUserProfiles) {
    ...
    try {
        ...
        # Determine cutoff date and enumerate candidate profiles (do not emit names to stdout)
        $cutoffDate = (Get-Date).AddDays(-$userProfileRetentionDays)
        $candidates = Get-ChildItem -Path "C:\Users" -Directory -ErrorAction SilentlyContinue | Where-Object {
            $_.Name -notin @('Default', 'Default User', 'Public', 'All Users') -and
            $_.PSIsContainer -and
            ((Get-ItemProperty -Path $_.FullName -ErrorAction SilentlyContinue).LastWriteTime -lt $cutoffDate)
        }

        $removedCount = 0
        $failedCount = 0
        foreach ($candidateProfile in $candidates) {
            try {
                # Log the full profile path and reason to the temp log only (Write-CustomMessage appends to temp log).
                Write-CustomMessage "USERPROFILE-LOG: Removing user profile at path: $($candidateProfile.FullName) (LastWriteTime: $($candidateProfile.LastWriteTime))"
                if (-not $DryRun) { Remove-Item -Path $candidateProfile.FullName -Recurse -Force -ErrorAction Stop }
                $removedCount++
            }
            catch {
                $failedCount++
                Write-CustomMessage "USERPROFILE-LOG: ERROR removing profile at path: $($candidateProfile.FullName): $_"
            }
        }

        # Emit a compact, non-PII summary to stdout/log for Intune consumption
        Write-CustomMessage ("User profile cleanup completed. RemovedCount={0}; FailedCount={1}" -f $removedCount, $failedCount)
    }
    catch {
    ...
    try {
        ...
        if ($_.Exception -is [System.UnauthorizedAccessException]) {
            ...
            try {
                ...
                if ($DryRun) {
                    foreach ($tf in $tempFiles) { Write-CustomMessage ("DRYRUN: Would remove (post-permission fix) temp item: {0}" -f $tf.FullName) }
                }
                else {
...
try {
    ...
    if (Test-Path -Path $tempLogFile) {
        # Read temp contents if present
        try {
            $tempContents = Get-Content -Path $tempLogFile -ErrorAction SilentlyContinue
        }
        catch { $tempContents = $null }

        if ($null -ne $tempContents -and $tempContents.Count -gt 0) {
            if ($DryRun) {
                # In DryRun we should still record what would have been done. Append a DRYRUN header
                # and the temp log contents into the persistent main log, but do not delete the temp file.
                try {
                    Add-Content -Path $logFile -Value "===== DRYRUN: Temp log contents from $tempLogFile at $(Get-Date -Format 'yyyy-MM-dd HH:mm:ss') =====" -ErrorAction SilentlyContinue
                    Add-Content -Path $logFile -Value $tempContents -ErrorAction SilentlyContinue
                    Write-CustomMessage ("DRYRUN: Appended temporary log contents into main log: $logFile (temp preserved)") -Level INFO
                }
                catch { Write-CustomMessage ("WARNING: Failed to append temp log into main log during DryRun: $_") }
            }
            else {
                try {
            ...
            # No temp contents found; if not DryRun and merged succeeded earlier, remove temp file
            if (-not $DryRun -and $merged) {
                try { Remove-Item -Path $tempLogFile -Force -ErrorAction Stop } catch { }
            }
        }
    }
}
...
try {
    # Run DISM component store cleanup as a final cleanup pass. Respect dry-run/what-if semantics so
    # callers who run the script in dry-run mode don't accidentally execute DISM.
    try {
        Write-CustomMessage "Invoking DISM component store cleanup (dry-run: $DryRun)."
        Invoke-DISMCleanup -WhatIf:$DryRun
    }
    catch {
        Write-CustomMessage ("ERROR: Invoke-DISMCleanup failed: {0}" -f $_.Exception.Message)
    }
    ...
    try {
        ...
        if ($PreserveTempLog -or (Test-Path -Path $tempLogFile)) {
